...
| Recruitment inbox stale-response race could overwrite newer results | deep admin concurrency audit | `requestVersion` + `detailVersion` guards prevent stale list/detail responses from mutating current state | Regression |
...
| Canonical route regressions (`/member`, `/scrims`, `/admin/scrims`) | historical routing fixes | `scripts/verify.mjs` asserts canonical destinations, redirects, breadcrumbs and sitemap routes | Regression |
| Admin preview button pointed at a redirect-only page | deep admin audit | `tests/regression/admin-surface.test.ts` + functional draft preview route | Regression + E2E |
...
| Image source drift could crash `next/image` after admin publish | deep admin asset audit | Image source unit tests + server publish validation + safe draft fallback | Unit + Regression |
