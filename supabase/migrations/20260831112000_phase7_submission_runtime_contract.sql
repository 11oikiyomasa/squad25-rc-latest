-- Phase 7 runtime submission contract.
-- Keeps the existing server-only v7 RPC as the single privileged write path.
-- No client INSERT capability is introduced.
...
begin
  ...
  if jsonb_typeof(body) <> 'object' then
    raise exception 'INVALID_APPLICATION_PAYLOAD' using errcode = '22023';
  end if;
  ...
  v_role := upper(btrim(coalesce(body->>'role', '')));
  v_nickname := btrim(coalesce(body->>'nickname', ''));
  v_resume_path := nullif(btrim(coalesce(body->>'resumePath', '')), '');
  v_resume_original_name := btrim(coalesce(body->>'resumeOriginalName', ''));
  ...
  if v_job is null or not exists (
    ...
    join public.recruitment_cycles c on c.id = j.cycle_id
    where j.id = v_job
      and c.status = 'OPEN'
      and j.is_active
  ...
  if char_length(v_cover) < 20 or char_length(v_cover) > 5000
     or char_length(v_portfolio) > 500
     or char_length(v_resume_original_name) < 1
     or char_length(v_resume_original_name) > 255 then
  ...
  if found
     and ip_record.window_started_at > now_ts - interval '1 hour'
     and ip_record.request_count >= 3 then
    raise exception 'RECRUITMENT_RATE_LIMIT'
      using errcode = 'P0001',
            detail = greatest(
              1,
              ceil(extract(epoch from (ip_record.window_started_at + interval '1 hour' - now_ts)))::integer
            )::text;
  end if;
