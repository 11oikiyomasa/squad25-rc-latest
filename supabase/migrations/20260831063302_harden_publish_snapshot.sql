...
begin
  ...
  if jsonb_typeof(payload->'achievements') <> 'array' then raise exception 'Content must contain an achievements array.' using errcode = '22023'; end if;
  if jsonb_typeof(payload->'gallery') <> 'array' then raise exception 'Content must contain a gallery array.' using errcode = '22023'; end if;
  ...
  for a in select * from jsonb_array_elements(payload->'achievements') with ordinality as x(item,ordinality) loop
    ...
    values(left(a.item->>'title',160),null,case when (a.item->>'year') ~ '^\d{4}$' then (a.item->>'year')::integer else null end,null,left(coalesce(a.item->>'description',''),600),null,a.ordinality-1);
  ...
  for g in select * from jsonb_array_elements(payload->'gallery') with ordinality as x(item,ordinality) loop
    ...
    values(left(g.item->>'title',160),left(g.item->>'image_url',800),left(coalesce(g.item->>'caption',''),300),g.ordinality-1);
    saved_gallery := saved_gallery + 1;
  end loop;

  return jsonb_build_object('ok',true,'savedMembers',saved_members,'savedMontages',saved_montages,'savedAchievements',saved_achievements,'savedGallery',saved_gallery);
end;
$$;
