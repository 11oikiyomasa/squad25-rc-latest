-- Prevent stale admin drafts from overwriting a newer published snapshot.
...
begin
  ...
  begin
    expected_updated_at := nullif(payload->>'expected_updated_at', '')::timestamptz;
  exception when others then
    raise exception 'A valid content revision is required.' using errcode = '22023';
  end;
  if expected_updated_at is null then raise exception 'A valid content revision is required.' using errcode = '22023'; end if;

  select updated_at into current_updated_at from squad_settings where id = 1 for update;
  if current_updated_at is null or current_updated_at <> expected_updated_at then
    raise exception 'Content changed since this draft was loaded.' using errcode = '40001';
  end if;
  ...
  for m in select * from jsonb_array_elements(payload->'members') loop
    ...
    update members set number=left(coalesce(m->>'number','00'),10),nickname=left(m->>'nickname',30),full_name=left(m->>'name',80),role=m->>'role',main_hero=left(coalesce(m->>'hero',''),50),status=m->>'status',bio=left(coalesce(m->>'bio',''),600),accent=left(coalesce(m->>'accent','#d7ff43'),20),photo_url=left(coalesce(m->>'photo',''),500),sort_order=greatest(0,(select ordinality-1 from jsonb_array_elements(payload->'members') with ordinality x(item,ordinality) where x.item=m limit 1)),updated_at=now_ts where id=db_member_id;
    ...
    for mt in select * from jsonb_array_elements(coalesce(m->'montages','[]'::jsonb)) with ordinality as x(item,ordinality) loop
      ...
      insert into montages(content_key,member_id,title,hero,duration,youtube_id,description,sort_order) values((m->>'id')||':'||mt.ordinality::text,db_member_id,left(mt.item->>'title',120),left(coalesce(mt.item->>'hero',m->>'hero',''),50),left(coalesce(mt.item->>'duration','00:00'),20),left(coalesce(mt.item->>'youtubeId',''),100),left(coalesce(mt.item->>'description',''),500),mt.ordinality-1)
