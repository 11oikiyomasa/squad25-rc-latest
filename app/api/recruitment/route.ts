...
const ROLES = new Set(['EXP', 'JUNGLE', 'MID', 'GOLD', 'ROAM', 'FLEX']); const MAX_MULTIPART_BYTES = 7 * 1024 * 1024;
function fail(message: string, status = 422) { return NextResponse.json({ error: message }, { status, headers: { 'Cache-Control': 'no-store' } }); }

async function sendConfirmationEmail(to: string, name: string, jobTitle: string) {
  const apiKey = process.env.RESEND_API_KEY, from = process.env.RECRUITMENT_FROM_EMAIL; if (!apiKey || !from) return;
  try { await fetch('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from, to: [to], subject: `Application received — ${jobTitle}`, html: `<p>Hi ${name.replace(/[<>&"']/g, '')},</p><p>We received your application for <strong>${jobTitle.replace(/[<>&"']/g, '')}</strong>. The team will review it and contact you if there is a next step.</p>` }), cache: 'no-store', signal: AbortSignal.timeout(7000) }); } catch (error) { console.error('Recruitment confirmation email failed:', error); }
}

export async function GET() {
  if (!isSupabaseConfigured()) return fail('Recruitment is temporarily unavailable.', 503);
  const supabase = await createClient(); const { data, error } = await supabase.from('recruitment_jobs').select('id,title,slug,description,requirements,closes_at').eq('is_active', true).order('created_at', { ascending: false });
  if (error) return fail('Recruitment is temporarily unavailable.', 503);
  const now = Date.now(); const jobs = (data ?? []).filter((job) => !job.closes_at || new Date(job.closes_at).getTime() > now);
