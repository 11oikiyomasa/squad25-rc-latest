...
import { assertPdf, clientIp, hasPdfMagicBytes, isValidEmail, isValidHttpUrl, text, verifyTurnstile } from '@/lib/recruitment-security';
  ...
  try {
    await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from,
        to: [to],
        subject: `Application received — ${safeTitle}`,
        html: `<p>Hi ${safeName},</p><p>We received your application for <strong>${safeTitle}</strong>. The team will review it and contact you if there is a next step.</p>`,
      }),
      cache: 'no-store',
      signal: AbortSignal.timeout(7000),
    });
  } catch (error) {
    console.error('Recruitment confirmation email failed:', error);
  }
}
...
export async function GET() {
  ...
  const now = Date.now();
  const jobs = (data ?? []).filter((job) => !job.closes_at || new Date(job.closes_at).getTime() > now);
  return NextResponse.json({ jobs }, { headers: { 'Cache-Control': 'public, max-age=60, s-maxage=60' } });
}
...
export async function POST(request: Request) {
  ...
  if (Number.isFinite(contentLength) && contentLength > MAX_MULTIPART_BYTES) {
    return fail('Payload terlalu besar. Resume maksimal 5 MB.', 413);
  }
  ...
  if (!jobId || fullName.length < 2 || fullName.length > 80 || nickname.length < 1 || !isValidEmail(email) || phone.length < 3 || !ROLES.has(role)) {
    return fail('Periksa nama, email, nomor telepon, dan role.');
  }
  if (portfolioLink && !isValidHttpUrl(portfolioLink)) return fail('Portfolio URL tidak valid.');
  if (!(resume instanceof File) || !assertPdf(resume) || !(await hasPdfMagicBytes(resume))) {
    return fail('Resume harus berupa PDF valid maksimal 5 MB.');
  }
  ...
  if (uploadError) {
    ...
    return fail('Resume gagal diupload. Coba lagi.', 500);
  }

  const { data: applicationId, error } = await admin.rpc('submit_recruitment_application_v7', {
    payload: {
      jobId,
      fullName,
      nickname,
  ...
  const { error: metadataError } = await admin
    ...
    .update({
      ...
      resume_original_name: resume.name.slice(0, 255),
      captcha_verified_at: new Date().toISOString(),
    })
    .eq('id', applicationId);
  if (metadataError) console.error('Recruitment resume metadata update failed:', metadataError.message);
