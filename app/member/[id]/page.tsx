        ...
        <div className="grid gap-6 lg:grid-cols-[.72fr_1.28fr]">
          <div className="relative min-h-[520px] overflow-hidden border border-white/10 bg-[var(--panel-raised)] lg:min-h-[720px]">
            ...
            <div className="absolute inset-x-0 bottom-0 p-6 sm:p-8">
              <div className="mb-3 font-mono text-[10px] uppercase tracking-[.24em] text-white/45">Player / {member.number}</div>
          ...
          <Card className="p-6 sm:p-8 lg:p-10">
            <div className="flex flex-wrap items-end justify-between gap-5 border-b border-white/8 pb-6">
              <div>
                <div className="font-mono text-[10px] uppercase tracking-[.22em] text-white/35">{member.name}</div>
              ...
              <div className="text-right">
                ...
                <div className="font-mono text-[9px] uppercase tracking-[.18em] text-white/30">cuts public</div>
              </div>
            </div>
            ...
            <div className="mt-10">
              <div className="mb-4 flex items-end justify-between gap-4 border-b border-white/8 pb-3">
                <div>
                  <div className="ui-eyebrow">Montage archive</div>
                  <h3 className="mt-1 text-lg font-semibold">Selected cuts</h3>
                </div>
                <div className="font-mono text-[9px] uppercase tracking-[.18em] text-white/25">{publishedCuts.length ? 'tap a cut to play' : 'no public cuts yet'}</div>
              </div>
              ...
              <div className="mt-6 grid gap-2 sm:grid-cols-2">
                {(() => {
                  ...
                  return (
                    <>
                      <a href={`/member/${previous.id}`} className="border border-white/8 p-4 transition-colors hover:border-white/18"><div className="font-mono text-[9px] uppercase tracking-[.18em] text-white/25">← Previous player</div><div className="mt-2 font-display text-2xl uppercase" style={{ color: previous.accent }}>{previous.nickname}</div></a>
                      <a href={`/member/${next.id}`} className="border border-white/8 p-4 text-right transition-colors hover:border-white/18"><div className="font-mono text-[9px] uppercase tracking-[.18em] text-white/25">Next player →</div><div className="mt-2 font-display text-2xl uppercase" style={{ color: next.accent }}>{next.nickname}</div></a>
                    </>
                  );
                })()}
              </div>
...
function Stat({ label, value }: { label: string; value: string }) {
  return <div className="border border-white/8 bg-black/15 p-4"><div className="font-mono text-[9px] uppercase tracking-[.18em] text-white/25">{label}</div><div className="mt-2 text-sm font-semibold">{value}</div></div>;
}
