...
export async function generateMetadata({ params }: { params: Promise<{ member_id: string }> }): Promise<Metadata> {
  ...
  if (!member) return { title: `Player not found — ${squadProfile.name}`, robots: { index: false, follow: false } };
  return {
    ...
    alternates: { canonical: `/roster/${member.id}` },
    openGraph: { title: `${member.nickname} / ${member.role} — ${profile.name}`, description: member.bio, type: 'website', url: `/roster/${member.id}`, images: member.photo ? [{ url: member.photo, alt: `${member.nickname} profile` }] : [] },
    ...
    <main className="min-h-screen overflow-x-clip bg-[var(--paper)] text-[var(--ink)]">
      ...
      <section className="ui-container pt-5 sm:pt-7">
        <nav aria-label="Breadcrumb" className="flex flex-wrap items-center gap-2 text-[9px] font-mono uppercase tracking-[.16em] text-white/30">
          <Link href="/" className="hover:text-white">Home</Link><span aria-hidden="true">/</span><Link href="/roster" className="hover:text-white">Roster</Link><span aria-hidden="true">/</span><span className="text-white/55">{member.nickname}</span>
        </nav>
      </section>
