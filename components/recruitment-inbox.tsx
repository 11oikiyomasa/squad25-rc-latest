...
async function fetchApplications(): Promise<Application[]> {
  ...
  return Array.isArray(payload?.applications) ? payload.applications as Application[] : [];
}
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        ...
        if (cancelled) return;
        const nextSelectedId = next.some((item) => item.id === selectedId) ? selectedId : next[0]?.id ?? '';
        const nextSelected = next.find((item) => item.id === nextSelectedId) ?? null;
        setApplications(next);
        setSelectedId(nextSelectedId);
        if (nextSelected) {
          setStatus(nextSelected.status);
          setNote(nextSelected.admin_note);
        }
      } catch (caught) {
        if (!cancelled) setError(caught instanceof Error ? caught.message : 'Failed to load applications.');
      } finally {
    ...
    <div className="grid gap-5 xl:grid-cols-[360px_1fr]">
      <aside className="border border-white/8 bg-[#101216] p-4">
        <div className="flex items-center justify-between gap-3 border-b border-white/8 pb-4">
          ...
          <button type="button" onClick={refresh} className="border border-white/10 px-3 py-2 text-[9px] uppercase tracking-[.16em] text-white/50 hover:border-white/25 hover:text-white">Refresh</button>
        </div>
