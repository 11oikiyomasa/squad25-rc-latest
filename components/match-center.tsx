    ...
    <section className="border-y border-white/8 bg-[var(--panel)]">
      <div className="ui-container py-16 sm:py-20 lg:py-24">
        <div className="flex flex-col gap-6 border-b border-white/8 pb-8 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <div className="ui-eyebrow">Match Center</div>
        ...
        <div className="mt-8 grid gap-3 lg:grid-cols-[1.35fr_.65fr]">
          <div className="border border-white/10 bg-[var(--panel-black)] p-5 sm:p-7">
            {next ? (
              <div className="grid gap-8 sm:grid-cols-[1fr_auto] sm:items-end">
                <div>
                  ...
                  <div className="mt-5 font-display text-5xl uppercase leading-none sm:text-7xl">SQUAD.25 <span className="text-white/20">vs</span> <span className="text-[var(--acid)]">{next.opponent_name}</span></div>
                ...
                <div className="border border-[color-mix(in_srgb,var(--acid)_20%,transparent)] bg-[color-mix(in_srgb,var(--acid)_5%,transparent)] px-4 py-3 text-center"><div className="font-mono text-[9px] uppercase tracking-[.18em] text-[var(--acid)]">{next.status === 'LIVE' ? 'LIVE' : 'LOCKED'}</div><div className="mt-1 font-display text-4xl text-[var(--acid)]">{formatTime(next.scheduled_at)}</div></div>
              </div>
            ) : (
          ...
          <div className="grid grid-cols-2 gap-px border border-white/10 bg-white/8">
            <div className="bg-[var(--panel-deep)] p-5 sm:p-6"><div className="font-display text-5xl">{results.length.toString().padStart(2, '0')}</div><div className="mt-2 font-mono text-[9px] uppercase tracking-[.18em] text-white/30">Results</div></div>
            <div className="bg-[var(--panel-deep)] p-5 sm:p-6"><div className="font-display text-5xl">{wins}-{losses}</div><div className="mt-2 font-mono text-[9px] uppercase tracking-[.18em] text-white/30">W / L</div></div>
            <div className="col-span-2 bg-[var(--panel-deep)] p-5 sm:p-6"><div className="flex items-end justify-between gap-4"><div><div className="font-mono text-[9px] uppercase tracking-[.18em] text-white/30">Public rooms</div><div className="mt-2 text-sm text-white/55">{upcoming.length} upcoming · {results.length} completed</div></div><div className="font-display text-4xl text-white/20">{scrims.length.toString().padStart(2, '0')}</div></div></div>
          </div>
        </div>

        {results.length > 0 && <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{results.slice(0, 3).map((scrim) => { const result = outcome(scrim); return <article key={scrim.id} className="border border-white/8 bg-[var(--panel-deep)] p-5"><div className="flex items-start justify-between gap-3"><div><div className="font-mono text-[9px] uppercase tracking-[.18em] text-white/25">{formatDate(scrim.scheduled_at)} / {scrim.format}</div><div className="mt-3 font-display text-2xl uppercase">{scrim.opponent_name}</div></div><div className={`font-mono text-[9px] font-black uppercase tracking-[.16em] ${result === 'WIN' ? 'text-[var(--acid)]' : result === 'LOSS' ? 'text-[var(--ember)]' : 'text-white/45'}`}>{result}</div></div><div className="mt-5 font-display text-4xl">{scrim.result_for} — {scrim.result_against}</div></article>; })}</div>}
      </div>
    </section>
  );
}
