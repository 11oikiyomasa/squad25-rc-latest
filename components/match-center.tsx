...
export default function MatchCenter({ scrims }: { scrims: Scrim[] }) {
  const upcoming = scrims
    .filter((scrim) => scrim.status === 'LIVE' || scrim.status === 'SCHEDULED')
    .sort((a, b) => new Date(a.scheduled_at).getTime() - new Date(b.scheduled_at).getTime());
  const next = upcoming[0] ?? null;
  const results = scrims
    .filter((scrim) => scrim.status === 'COMPLETED' && scrim.result_for !== null && scrim.result_against !== null)
      ...
      <div className="mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-24">
        <div className="flex flex-col gap-6 border-b border-white/8 pb-8 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-4 max-w-xl text-sm leading-6 text-white/45">Published scrims, upcoming rooms, and receipts from the last block.</p>
          </div>
          ...
          <div className="border border-white/10 bg-black p-5 sm:p-7">
            {next ? (
              <div className="grid gap-8 sm:grid-cols-[1fr_auto] sm:items-end">
                <div>
                  <div className="flex flex-wrap items-center gap-2 font-mono text-[9px] uppercase tracking-[.18em] text-white/35">
                    <span>{next.status === 'LIVE' ? 'LIVE NOW' : 'NEXT SCRIM'}</span><span>•</span><span>{date(next.scheduled_at)} / {time(next.scheduled_at)} WIB</span><span>•</span><span>{next.format}</span>
                  </div>
                ...
                <div className="border border-[#d7ff43]/20 bg-[#d7ff43]/[.05] px-4 py-3 text-center">
                  <div className="font-mono text-[9px] uppercase tracking-[.18em] text-[#d7ff43]">{next.status === 'LIVE' ? 'LIVE' : 'LOCKED'}</div>
                  <div className="mt-1 font-display text-4xl text-[#d7ff43]">{time(next.scheduled_at)}</div>
                </div>
              </div>
            ) : (
              <div className="flex min-h-52 items-center justify-between gap-5">
                <div><div className="font-mono text-[9px] uppercase tracking-[.18em] text-white/25">No next match</div><div className="mt-3 font-display text-4xl uppercase">Schedule is open.</div><p className="mt-3 max-w-xl text-sm leading-6 text-white/40">No public scrim has been locked. The match center will update automatically when one is published.</p></div>
              </div>
            )}
          </div>
        ...
        {results.length > 0 && (
          <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {results.slice(0, 3).map((scrim) => {
              const result = outcome(scrim);
              return <article key={scrim.id} className="border border-white/8 bg-[#0e1013] p-5"><div className="flex items-start justify-between gap-3"><div><div className="font-mono text-[9px] uppercase tracking-[.18em] text-white/25">{date(scrim.scheduled_at)} / {scrim.format}</div><div className="mt-3 font-display text-2xl uppercase">{scrim.opponent_name}</div></div><div className={`font-mono text-[9px] font-black uppercase tracking-[.16em] ${result === 'WIN' ? 'text-[#d7ff43]' : result === 'LOSS' ? 'text-[#ff6b38]' : 'text-white/45'}`}>{result}</div></div><div className="mt-5 font-display text-4xl">{scrim.result_for} — {scrim.result_against}</div></article>;
            })}
          </div>
        )}
      </div>
