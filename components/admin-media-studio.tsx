...
export default function AdminMediaStudio() {
  ...
  useEffect(() => {
    ...
    fetch('/api/admin/content', { cache: 'no-store' })
      ...
      .catch((error) => { if (!cancelled) setMessage(error instanceof Error ? error.message : 'Unable to load media.'); })
  ...
  function save(next: ContentState) {
    ...
    try { localStorage.setItem(DRAFT_KEY, serialized); setMessage('Media draft saved locally.'); }
  ...
  return <main className="min-h-screen bg-[#0c0d0f] text-[#f4f0e7]"><section className="mx-auto max-w-7xl px-5 py-8 lg:px-8 lg:py-12">
    <div className="flex flex-wrap items-end justify-between gap-5 border-b border-white/8 pb-6"><div><div className="text-[9px] uppercase tracking-[.22em] text-[#d7ff43]">Content Studio / Media</div><h1 className="mt-2 font-display text-6xl uppercase leading-none sm:text-8xl">Archive room.</h1><p className="mt-4 max-w-2xl text-sm leading-6 text-white/45">Edit gallery and achievement metadata, review the draft, then publish the complete snapshot.</p></div><div className="flex items-center gap-2"><span className={`text-[9px] uppercase tracking-[.15em] ${dirty ? 'text-[#ff6b38]' : 'text-white/25'}`}>{dirty ? 'Unsaved draft' : 'Published'}</span><button type="button" disabled={!dirty || busy} onClick={() => void publish()} className="bg-[#d7ff43] px-4 py-3 text-[9px] font-black uppercase tracking-[.16em] text-black disabled:cursor-not-allowed disabled:opacity-35">{busy ? 'Publishing…' : 'Publish'}</button></div></div>
    {message && <div className="mt-5 border border-white/8 bg-white/[.02] px-4 py-3 text-xs text-white/55">{message}</div>}
    <div className="mt-8 grid gap-8 xl:grid-cols-[1.25fr_.75fr]"><section className="border border-white/8 bg-[#101216] p-5 sm:p-6"><div className="flex items-end justify-between gap-4 border-b border-white/8 pb-4"><div><div className="text-[9px] uppercase tracking-[.2em] text-[#ff6b38]">Gallery archive</div><h2 className="mt-1 text-xl font-semibold">{data.gallery.length} items</h2></div><button type="button" onClick={addGallery} className="border border-white/10 px-3 py-2 text-[9px] font-black uppercase tracking-[.16em] text-white/55">+ Add</button></div><div className="mt-5 space-y-4">{data.gallery.map((item, index) => <article key={item.id || index} className="grid gap-4 border border-white/8 p-4 lg:grid-cols-[150px_1fr]"><div className="relative aspect-[4/3] overflow-hidden border border-white/8 bg-black">{isAllowedImageSource(item.image) ? <Image src={item.image} alt={item.title} fill sizes="150px" className="object-cover" /> : <div className="grid h-full place-items-center px-3 text-center text-[9px] uppercase tracking-[.15em] text-white/20">Missing or unsupported image</div>}</div><div className="grid gap-3"><Field label="Title" value={item.title} onChange={(value) => updateGallery(index, { title: value })}/><Field label="Caption / Meta" value={item.meta} onChange={(value) => updateGallery(index, { meta: value })}/><Field label="Image URL" value={item.image} onChange={(value) => updateGallery(index, { image: value })}/><button type="button" onClick={() => removeGallery(index)} className="justify-self-start text-[9px] uppercase tracking-[.16em] text-[#ff6b38]">Remove item</button></div></article>)}</div></section><section className="border border-white/8 bg-[#101216] p-5 sm:p-6"><div className="flex items-end justify-between gap-4 border-b border-white/8 pb-4"><div><div className="text-[9px] uppercase tracking-[.2em] text-[#d7ff43]">Achievements</div><h2 className="mt-1 text-xl font-semibold">{data.achievements.length} records</h2></div><button type="button" onClick={addAchievement} className="border border-white/10 px-3 py-2 text-[9px] font-black uppercase tracking-[.16em] text-white/55">+ Add</button></div><div className="mt-5 space-y-4">{data.achievements.map((achievement, index) => <article key={`${achievement.year}-${achievement.title}-${index}`} className="border border-white/8 p-4"><div className="grid gap-3 sm:grid-cols-[100px_1fr]"><Field label="Year" value={achievement.year} onChange={(value) => updateAchievement(index, { year: value })}/><Field label="Title" value={achievement.title} onChange={(value) => updateAchievement(index, { title: value })}/><div className="sm:col-span-2"><label className="block"><span className="text-[9px] uppercase tracking-[.18em] text-white/25">Note</span><textarea value={achievement.note} onChange={(event) => updateAchievement(index, { note: event.target.value })} className="mt-2 min-h-24 w-full border border-white/10 bg-[#0d0f11] p-3 text-xs leading-6 text-white outline-none focus:border-[#d7ff43]/35"/></label></div></div><button type="button" onClick={() => removeAchievement(index)} className="mt-3 text-[9px] uppercase tracking-[.16em] text-[#ff6b38]">Remove record</button></article>)}</div></section></div>
