...
function publishedMontages(member: Member | null) {
  return member?.montages.filter((montage) => Boolean(normalizeYoutubeId(montage.youtubeId))) ?? [];
}

function initialPublicMontage(member: Member | null, initialMontageIndex: number) {
  if (!member) return null;
  const requested = member.montages[initialMontageIndex];
  if (requested && normalizeYoutubeId(requested.youtubeId)) return requested;
  return publishedMontages(member)[0] ?? null;
}
