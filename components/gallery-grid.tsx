      ...
      <div className="mt-8 grid auto-rows-[190px] gap-3 sm:grid-cols-2 lg:grid-cols-12 lg:auto-rows-[150px]">
        {items.map((item, index) => (
          <button
            ...
            className={`group relative overflow-hidden border border-white/8 bg-[#14171b] text-left focus-visible:outline-2 focus-visible:outline-[var(--focus-ring)] focus-visible:outline-offset-2 ${index === 0 ? 'sm:row-span-2 lg:col-span-5 lg:row-span-4' : index === 1 ? 'lg:col-span-4 lg:row-span-3' : index === 2 ? 'lg:col-span-3 lg:row-span-3' : index === 3 ? 'lg:col-span-4 lg:row-span-2' : index === 4 ? 'lg:col-span-3 lg:row-span-2' : 'lg:col-span-5 lg:row-span-3'}`}
          >
            ...
            <div className="absolute inset-x-0 bottom-0 p-4 sm:p-5"><div className="font-mono text-[9px] uppercase tracking-[.18em] text-white/40">{item.meta}</div><div className="mt-1 text-sm font-semibold uppercase tracking-[.06em]">{item.title}</div></div>
          </button>
        ))}
      </div>
