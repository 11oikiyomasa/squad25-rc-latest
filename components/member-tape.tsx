  ...
  return (
    <div className="space-y-3">
      {montages.map((montage, index) => {
        ...
        const isPlaying = active === youtubeId && Boolean(youtubeId);
        return youtubeId ? (
          <article key={`${montage.title}-${index}`} className="overflow-hidden border border-white/8 bg-white/[.02]">
            {isPlaying ? (
              ...
              <button type="button" onClick={() => setActive(youtubeId)} className="group relative block aspect-video w-full overflow-hidden bg-black text-left sm:aspect-[2.25/1]">
